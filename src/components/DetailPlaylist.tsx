  ...
  return (
    <>
      <div className="playlist_info flex gap-4 items-center justify-center py-6">
        <div className="thumbnail flex-shrink-0 w-1/5">
          <img src={info?.images[0].url} alt={info?.name} className="w-full h-auto" />
        </div>

        <div className="info flex flex-col gap-3">
          <div className="playlist_title text-4xl font-bold">{info?.name}</div>
          <div className="playlist_description ">{info?.description}</div>
        </div>
      </div>
      ...
      <ul className="flex flex-col gap-4 overflow-y-auto max-h-96">
        {item?.map((song, idx) => {
          return (
            <li key={song.track.id} className="flex items-center gap-4">
              <div className="no flex items-center justify-center flex-shrink-0 w-6 h-6">{idx + 1}</div>
              ...
              <Link href={`https://open.spotify.com/album/${song.track.album.id}`} target="_blank">
                {song.track.album.name}
              </Link>

              <div className="playtime flex justify-center w-24 ml-auto">{convertTime(song.track.duration_ms)}</div>
            </li>
          );
        })}
      </ul>
